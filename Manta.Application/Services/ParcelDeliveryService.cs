...
//     }
//
//     public async Task<Guid> AcceptedAtDeliveryPoint(Guid deliveryPointId, Guid parcelId, Guid userId)
//     {
//         var parcel = await _parcelRepository.GetByIdAsync(parcelId) ??
//                      throw new ArgumentException($"Parcel with id {parcelId} not found.");
...
//                 await UnloadFromDeliveryVehicle(parcel.CurrentVehicleId, parcel.Id, user);
//             parcel.MoveToLocation(deliveryPoint.Id);
//             await _parcelRepository.UpdateAsync(parcel);
//             await _parcelRepository.SaveChangesAsync();
//             return parcel.Id;
//         }
//         else throw new ArgumentException("Failed to apply status rule for parcel", nameof(parcel));
//     }
//
