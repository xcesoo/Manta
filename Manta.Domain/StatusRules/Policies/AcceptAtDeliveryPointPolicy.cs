...
public class AcceptAtDeliveryPointPolicy : IParcelStatusRule
{
    private readonly WrongDeliveryPointRule _wrong = new ();
    private readonly CanceledToReturnRequestedRule _cancelToReturn = new ();
    ...
    public RuleResult ShouldApply(RuleContext context)
    {
        if (context.DeliveryPoint is null)
            return RuleResult.Failed(ERuleResultError.ArguementInvalid, "DeliveryPoint is required for acceptance policy.");
